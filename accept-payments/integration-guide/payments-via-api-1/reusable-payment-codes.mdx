...
import CreatePrReusablePaymentCode from '/snippets/payments-api/create-pr-reusable-payment-code.mdx'
import PaymentRequestWebhookRpc from '/snippets/payments-api/payment-request-webhook-rpc.mdx'
...
sequenceDiagram
  ...
  Client ->> EndUser: Retrieve payment code
  EndUser ->> XenditServer: Makes payment to <br> the payment code
...
```mermaid
stateDiagram-v2
    [*] --> ACCEPTING_PAYMENTS: Request created
    ACCEPTING_PAYMENTS --> ACCEPTING_PAYMENTS: End user payment captured
    ACCEPTING_PAYMENTS --> CANCELED: Merchant cancels request
    ACCEPTING_PAYMENTS --> EXPIRED: Request reaches expiry
```
