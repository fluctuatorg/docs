...
sequenceDiagram
    ...
    participant AccountA as Tokenizing account (master or sub-account)
    participant AccountB as Charging account (master or sub-account)
    participant Xendit

    Customer->>AccountA: Submit card details
    AccountA->>Xendit: Create token (is_multiple_use=true)
    Xendit-->>AccountA: Return credit_card_token_id
    AccountB->>Xendit: Authenticate/charge with shared token
    Xendit-->>AccountB: Charge result
```
